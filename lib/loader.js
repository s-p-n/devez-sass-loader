...
function sassLoader(content) {
    if (asyncSassJobQueue === null) {
        const sass = require("devez-node-sass");
        const sassVersion = /^(\d+)/.exec(require("devez-node-sass/package.json").version).pop();

        if (Number(sassVersion) < 1) {
            throw new Error(
                "The installed version of `devez-node-sass` is not compatible (expected: >= 1, actual: " + sassVersion + ")."
            );
        }
