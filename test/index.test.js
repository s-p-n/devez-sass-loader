    ...
    describe("errors", () => {
        ...
        it("should not swallow errors when trying to load node-sass", (done) => {
            ...
            runWebpack({
                ...
                mockRequire.reRequire("devez-node-sass");
                err.message.should.match(/Some error/);
                done();
            });
        });
        it("should output a message when `devez-node-sass` is an incompatible version", (done) => {
            mockRequire.reRequire(pathToSassLoader);
            mockRequire("devez-node-sass/package.json", { version: "0.0.0" });
            runWebpack({
                entry: pathToSassLoader + "!" + pathToErrorFile
            }, (err) => {
                mockRequire.stop("devez-node-sass");
                err.message.should.match(/The installed version of `devez-node-sass` is not compatible/);
                done();
            });
        });
    });
